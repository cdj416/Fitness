...
import com.hongyuan.fitness.custom_view.LineChartMarkView;
...
//        description.setText("需要展示的内容");
        ...
        leftYAxis.setGridColor(0xFFFFFFFF);
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<SaleDataBean.DataBean.ChartListBean> dataList, String name, int color) {
        final DecimalFormat df = new DecimalFormat("0.00");
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            SaleDataBean.DataBean.ChartListBean data = dataList.get(i);
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i, (float) data.getAmount());
            entries.add(entry);
        }

        /******根据需求的不同 在此在次设置X Y轴的显示内容******/
        xAxis.setLabelCount(dataList.size()-1, false);
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                String tradeDate="";
                    if((int) value % dataList.size()==0||(int) value % dataList.size()==11){
                        tradeDate = dataList.get((int) value % dataList.size()).getTime();
                    }
                return tradeDate;
            }
        });
    ...
    * 收益使用的
    * */
    /*public void showIconLineChart(final List<IncomeDataBean.DataBean.EveryDataListBean> dataList, String name, int color) {
        final DecimalFormat df = new DecimalFormat("0.00");
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            IncomeDataBean.DataBean.EveryDataListBean data = dataList.get(i);
            *//**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             *//*
            Entry entry = new Entry(i, (float) data.getReturnedData());
            entries.add(entry);
        }
        ...
        *//******根据需求的不同 在此在次设置X Y轴的显示内容******//*
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                String tradeDate="";
                if((int) value % dataList.size()==0||(int) value % dataList.size()==11){
                    tradeDate = dataList.get((int) value % dataList.size()).getTranTimesFormat()+"";
                }
                return tradeDate;
            }
        });
